...
function checkWinner(player, string) {
    for (let combination of winningCombinations) {
        if (
            combination[0].textContent === player && combination[1].textContent === player && combination[2].textContent === player
        ) {
...
function turnIndicator() {
    if (!checkWinner(character, "Player 1") && !checkWinner(character2, "Player 2")) {
        if (round % 2 === 0) {
            turnScreen.textContent = "Player 1's turn";
        } else {
...
function randomChoice() {
    ...
    round += 1;
    turnIndicator();
    if (round === 9 && !checkWinner(character, "Player1") && !checkWinner(character2, "Player 2")) {
        handleDraw();
    }
    setTimeout(() => {
    ...
    button.addEventListener("click", function () {
        if (round % 2 === 0) {
            ...
            if (!withPerson && !checkWinner(character, "Player 1")) {
                setTimeout(() => {
                    const bestMove = minimax([...buttons].map(button => button.textContent), 0, true);
        ...
        if (round === 9 && !checkWinner(character, "Player 1") && !checkWinner(character2, "Player 2")) {
            handleDraw();
        }
    });
});
